...
def grid_search(data, param = {'alpha':[1],'factors': [10],'regularization': [0.01]},n_threads = 0,
    ...
    TODO:
        1) Parallelize
        2) Create own class 
        3) Integrate alpha parameter to implicit library
    ...
    **kwargs: Arguments for the train_test function
    ...
    #Optimal parameters
    min_metric = d_param.values()[0]
    for i in xrange(1,len(d_param.keys())):
        if d_param.values()[i]['rmse_ave']<min_metric['rmse_ave']:
            min_metric = d_param.values()[i]
